...
- **ผลข้างเคียงของ `print-color-adjust:exact`:** พื้นหลัง *ทุกจุด* ถูกพิมพ์จริง รวมถึงตัว `.paper` เอง → `--p-paper` ต้องเป็น `#fff` สนิท (เดิม `#FCFCFA` ออกมาเป็นคราบจางทั้งแผ่น) และแถวเปล่า `.empty-cell` ต้อง `background:#fff` ทับซีบร้า `nth-child(even)` ไม่งั้นได้แถบมิ้นต์จางในช่องว่าง
...
3. **กระดาษขาวสะอาดตอนพิมพ์** (ฟีดแบ็ก) — พื้นแผ่นมีคราบสีจางเพราะ `--p-paper:#FCFCFA` + `print-color-adjust:exact` พิมพ์พื้นออกมาจริง → เปลี่ยนเป็น `#fff` และให้แถวเปล่า `.empty-cell` เป็นขาว (ซีบร้าเหลือเฉพาะแถวที่มีรายการจริง)
