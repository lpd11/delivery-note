...
- **POST ไป Apps Script "สำเร็จแต่คำตอบหาย" ได้** — /exec ตอบ 302 ให้ไปดึงผลที่ googleusercontent; บนมือถือบางครั้ง redirect เพี้ยนกลับมาที่ /exec แบบ GET → doGet ไม่มี action → client เห็น `Invalid GET action: undefined` **ทั้งที่แถวถูกเขียนลงชีตแล้ว** (เจอจริง 28 ก.ค. 2569: บิล 6907-003 createdAt 20:47:50 แต่ผู้ใช้เห็น error 20:48). ห้ามให้ผู้ใช้กดบันทึกซ้ำเพราะจะได้บิลซ้ำ → `verifySaved()` ใน bill.js / เช็ค `getBill().bill.status` ใน history.js จะยืนยันกับฐานข้อมูลก่อนค่อยแจ้ง error
- **api.js ไม่ใช้ `alert()` แล้ว** (ตัดออก 29 ก.ค.) — ทุก caller จับ error เองและขึ้น toast; อย่าเอา alert กลับมา
...
## สถานะ (29 ก.ค. 2569 / 2026) — DEPLOY แล้ว ใช้งานจริงอยู่ ✅
...
- [x] **Deploy backend จริง** — deployment เดิม redeploy เป็น `@3` (29 ก.ค.), ยิง `getBills` สดผ่าน, บิลเก่าอ่านเป็น `status:active` ถูกต้อง
- [x] **Deploy GitHub Pages** → https://lpd11.github.io/delivery-note/
- [x] ใช้งานจริงแล้ว — บิลจริงในระบบ 6907-001 / 6907-002 (voided แล้ว = user ลองปุ่มยกเลิกจริง ผ่าน) / 6907-003
- [ ] ใส่โลโก้จริงของร้าน (ตอนนี้ใช้ไอคอนบ้าน+เค้กชั่วคราว) — อัปโหลดผ่านหน้า "ตั้งค่า"
- [ ] ยังไม่ได้ยืนยันปุ่ม "แก้ไข" กับบิลจริงบนมือถือ (ยกเลิกยืนยันแล้วจาก 6907-002)
...
## Changelog — session 25 ก.ค. 2569 (2026)
...
## Changelog — session 29 ก.ค. 2569 (2026)

1. **กระดาษขาวสะอาดตอนพิมพ์** (ฟีดแบ็ก) — พื้นแผ่นมีคราบสีจางเพราะ `--p-paper:#FCFCFA` + `print-color-adjust:exact` พิมพ์พื้นออกมาจริง → เปลี่ยนเป็น `#fff`; เคยลองให้แถวเปล่าเป็นขาวด้วยแต่ user เลือกคงซีบร้าไว้ทั้งตาราง
2. **แก้ไขบิล + ยกเลิกบิล** — ปุ่มบนการ์ดหน้าประวัติ; backend เพิ่ม `updateBill_`/`voidBill_`/`unvoidBill_` + คอลัมน์ `status`/`voidedAt`/`updatedAt` (ensureColumns_ migrate ชีตเดิมเอง); หน้าออกบิลรับ `?billId=` เข้าโหมดแก้ไข (แบนเนอร์ + ปุ่มเปลี่ยนเป็น "บันทึกการแก้ไข"); หน้าพิมพ์ประทับตรา "ยกเลิก" + ซ่อนปุ่มพิมพ์เมื่อบิล voided. เลือก "ยกเลิก" แทน "ลบจริง" เพราะเลขบิลจะได้ไม่หายไปจากลำดับ (สำคัญกับใบเสร็จที่มีเลขผู้เสียภาษี). ทดสอบ mock 19 เคสผ่านด้วย Node (vm + localStorage จำลอง ไม่ต้องใช้ jsdom เพราะ api.js ไม่แตะ DOM)
3. **กัน error หลอกตอนบันทึก** (จากภาพหน้าจอ user 28 ก.ค.) — POST สำเร็จแต่คำตอบหายกลางทาง ทำให้เด้ง alert `Invalid GET action: undefined` ทั้งที่บิล 6907-003 เข้าชีตแล้ว (createdAt 20:47:50 vs error 20:48) → ตัด `alert()` ออกจาก api.js + เพิ่ม `verifySaved()` (bill.js) และเช็ค status จริง (history.js) ก่อนแจ้ง error กันผู้ใช้กดซ้ำจนได้บิลซ้ำ
